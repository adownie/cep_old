...
class HeadwayLayout {
	...
	public static function get_status($layout, $include_post_status = false) {
		...
		$customized = ( HeadwaySkinLayoutOption::get($layout, 'customized') === true ) ? true : false;	
		$template = false;

		if ( $possible_template = HeadwaySkinLayoutOption::get($layout, 'template') ) {
	...
	public static function get_pages() {
		...
		public static function get_pages_posts($post_type = 'post', $post_parent = 0) {
			...
			/* If it's a post then remove the post parent 0 argument because there are awful plugins out there that add post parents to posts which is not a hierarchical post type. */
			if ( $post_type == 'post' )
			...
			foreach ( $query as $post ) {
				...
				/* Use ternary operator here to stop infinite loop since it would constantly query for posts since we're ignoring post parent */
				$posts['single-' . $post_type . '-' . $post->ID] =  $post_type != 'post' ? self::get_pages_posts($post_type, $post->ID) : array();
	...
	public static function delete_layout($layout_id, $skin_only = true) {
		...
		//Delete wrappers and wrapper instances
			$layout_wrappers = HeadwaySkinOption::get($layout_id, 'wrappers', array());

			if ( is_array($layout_wrappers) ) {

				foreach ( $layout_wrappers as $wrapper_id => $wrapper_settings ) {
					HeadwayElementsData::delete_special_element_properties(null, 'wrapper', 'instance', $wrapper_id . '-layout-' . $layout_id);
				}

			}
	
			HeadwaySkinOption::delete($layout_id, 'wrappers');

		//Delete the layout options for this layout
		if ( $skin_only ) {
			HeadwaySkinLayoutOption::delete_all_from_layout($layout_id);
		} else {
