...
/* IFRAME FUNCTIONS */
	...
	waitForIframeLoad = function(callback, iframe) {
		...
		/* Fire callback! */
		return iframeLoadCallback(callback);

	}

		iframeLoadCallback = function(callback) {
		...
		/* Bindings with modifier */
			...
			/* Design Editor Stuff */
				if ( typeof designEditor != 'undefined' && typeof designEditor.processElementCopy == 'function' ) {
					...
					/* Copy and Paste */
						$(document).bind('keyup', 'ctrl+c', designEditor.processElementCopy);
						$i('html').bind('keyup', 'ctrl+c', designEditor.processElementCopy);

						$(document).bind('keyup', 'ctrl+v', designEditor.processElementPaste);
						$i('html').bind('keyup', 'ctrl+v', designEditor.processElementPaste);

					/* Live CSS Toggle */
						$(document).bind('keyup', 'ctrl+e', function() { if ( !boxOpen('live-css') ) { $('#open-live-css').trigger('click'); } else { closeBox('live-css'); } });
						$i('html').bind('keyup', 'ctrl+e', function() { if ( !boxOpen('live-css') ) { $('#open-live-css').trigger('click'); } else { closeBox('live-css'); } });
					...
					/* Design Editor Toggle */
						$(document).bind('keyup', 'tab', toggleDesignEditor);
						$i('html').bind('keyup', 'tab', toggleDesignEditor);
	...
	updateBlockContentCover = function(block) {
		...
		block.append('\
				<div class="block-content-cover">\
					...
					\
					<span class="block-type-and-id">' + blockTypeNice + ' <span>#' + block.data('id') + '</span></span>\
				</div>\
			');
			...
			blockDimensionsTooltipContent = function(api) {
				...
				/* Block Info (only if existing block) */
					if ( typeof blockType != 'undefined' ) {
						...
						var blockMirrored = isBlockMirrored(block) ? '<span class="block-info-mirroring">Mirroring #' + getBlockMirrorOrigin(block) + blockMirroredLayoutName + '</span>' : '';
...
/* PANEL FUNCTIONS */
	...
	addPanelScrolling = function() {
		...
		$('ul.sub-tabs').perfectScrollbar();
		$('div.sub-tabs-content-container').perfectScrollbar();
	...
	/* DESIGN EDITOR DATA */
		...
		dataSetDesignEditorProperty = function(args) {
			...
			/* Update the properties in the tree */
				if ( typeof designEditor != 'undefined' && $('ul#design-editor-element-selector').hasClass('element-properties-visible') ) {

					designEditor.showElementPropertiesThrottled($('ul#design-editor-element-selector li.element[data-element-id="' + element + '"]'));
