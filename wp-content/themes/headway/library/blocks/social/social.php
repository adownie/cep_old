...
class HeadwaySocialBlock extends HeadwayBlockAPI {
		...
		if ($orientation === 'vertical') {
			...
			(function ($) {
				$(function() {
					...
					var iconsContainer  = $("#block-' . $block_id . ' ul");
					var icons    		= iconsContainer.find("li");
					var images    		= iconsContainer.find("li img");
					
					/* Get width of highest icon */
					...
					images.on("load",function(){
						...
						images.each(function() {
							...
							iconsContainer.width(maxWidth)
						});

						
						/* Set the width */
						var maxHeight = iconsContainer.outerHeight();
						if (maxHeight) {
						  	iconsContainer.height(maxHeight).css("position", "absolute");
						}	
			...
			(function ($) {
				$(function() {
					...
					images.on("load",function(){
						...
						/* Set the height */
						if (maxHeight)
							iconsContainer.height(maxHeight);

						/* Set the width */
						iconsContainer.width("auto");//Fix to force width to calculate correctly
						var iconsWidth = iconsContainer.outerWidth();
						iconsContainer.width(iconsWidth).css("position", "absolute");
	...
	public static function dynamic_css($block_id, $block = false) {
		...
		/* Set positioning CSS for multiple images */
		if ($position) {
			$css .= '
	...
	function content($block) {
		...
		$icon_set = HeadwayBlockAPI::get_setting($block, 'icon-set', 'custom');
	...
	function add_uploader_svg_mime( $mimes ){
...
class HeadwaySocialBlockOptions extends HeadwayBlockOptionsAPI {
	...
	function modify_arguments($args = false) {
		...
		if ($icon_set != 'custom') {
			$this->tabs[$icon_set] = ucwords(str_replace('-', ' ', $icon_set));

			$this->inputs[$icon_set] = array(
	...
	function get_icon_sets() {
		...
		$icons_options = array('custom' => 'Custom Icons');
