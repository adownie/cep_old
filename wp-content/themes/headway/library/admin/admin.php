...
class HeadwayAdmin {
	...
	public static function form_action_reset() {
		...
		/* Set these so stray skin cache data doesn't get left behind */
		HeadwayOption::$current_skin = HEADWAY_DEFAULT_SKIN;
		HeadwayLayoutOption::$current_skin = HEADWAY_DEFAULT_SKIN;
		...
		foreach ( wp_load_alloptions() as $option => $option_value ) {
			...
			if ( strpos($option, 'headway_option_') === 0 || strpos($option, 'headway_layout_options_') === 0 || strpos($option, 'headway_|skin=') === 0 || $option === 'headway' ) {
	...
	public static function add_menus(){
		...
		$icon = (version_compare($GLOBALS['wp_version'], '3.8', '>=') && get_user_option('admin_color') != 'light') ? 'headway-16-white.png' : 'headway-16.png';
