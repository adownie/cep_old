...
class HeadwaySkin {
	...
	function register($id, $name, $path, $options = false){
		...
		if(($this->is_active() && !isset($_GET['skin-preview'])) || ($_GET['skin-preview'] == $this->id && headway_can_visually_edit())) $this->exec();
	}
	...
	function exec(){
		...
		if(headway_json_encode(headway_get_option('skin-options')) != headway_json_encode($this->options)){
	 ...
	 **/
	function is_active(){
		if(headway_get_option('active-skin') == $this->id){
	...
	function create_thumbnail(){
		$thumbnail = (file_exists($this->abspath.'/screenshot.jpg')) ? $this->get_url().'/screenshot.jpg' : get_bloginfo('template_directory').'/library/visual-editor/images/default_thumbnail.jpg';
	...
	function create_text_input($options){
		...
		$return .= '<th scope="row"><label for="skin_option_'.str_replace('-', '_', $options['name']).'">'.$options['label'].'</label></th>
						<td><input type="text" name="skin-options['.$options['name'].']" id="skin_option_'.str_replace('-', '_', $options['name']).'" value="'.htmlentities(headway_skin_option($options['name'], true)).'" />';
	...
	function create_textarea($options){
		...
		$return .= '<th scope="row"><label for="skin_option_'.str_replace('-', '_', $options['name']).'">'.$options['label'].'</label></th>
						<td><textarea class="regular-text" name="skin-options['.$options['name'].']" id="skin_option_'.str_replace('-', '_', $options['name']).'" rows="6">'.htmlentities(headway_skin_option($options['name'], true)).'</textarea>';
 ...
 * @return mixed
 **/
function headway_get_skin_option($option){	
	if(headway_is_skin_active())
		$skin_options = headway_get_option('skin-options', true, true);
	
	if(isset($skin_options[$option]) && $skin_options['skin'] == headway_get_option('active-skin', true, true)){
...
function headway_reset_active_skin(){	
	if(!headway_active_skin() && headway_get_option('active-skin') != ''){
 ...
 * Headway leafs API.
 * 
 * @todo Ability to add to the advanced leafs panel.  Semi-started, but not finished.
 *
 * @package Headway
 * @subpackage Leafs
 **/
class HeadwayLeaf {	
	...
	function content($leaf){
		if($this->show_hooks){
			do_action('headway_leaf_content_top');
			do_action('headway_leaf_content_top_'.$leaf['id']);
		}
		...
		if($this->show_hooks){
			do_action('headway_leaf_content_bottom');
			do_action('headway_leaf_content_bottom_'.$leaf['id']);
		}
	}
	...
	function options($leaf_id, $get_width = false){
		if(!$get_width){
			...
			echo str_replace('px', '', $this->options_width);
		}
	}
...
class HeadwayLeafsHelper {
	...
	function create_text_input($options, $leaf_id, $type = 'leaf-options'){
		...
		echo '<th scope="row"><label for="'.$leaf_id.'_'.str_replace('-', '_', $options['name']).'">'.$options['label'].'</label></th>
						<td><input type="text" class="headway-visual-editor-input" name="'.$type.'['.$leaf_id.']['.$options['name'].']" id="'.$leaf_id.'_'.str_replace('-', '_', $options['name']).'" value="'.htmlentities($options['value']).'" /></td>';
	...
	function create_textarea($options, $leaf_id, $type = 'leaf-options'){
		...
		echo '<tr class="no-border">
				...
				<tr class="textarea'.$no_border.'">
					<td colspan="2"><textarea class="text-content headway-visual-editor-input" name="'.$type.'['.$leaf_id.']['.$options['name'].']" id="'.$leaf_id.'_'.str_replace('-', '_', $options['name']).'">'.htmlentities($options['value']).'</textarea></td>
			  </tr>';
...
function headway_add_custom_font($name, $family){
	...
	$headway_custom_fonts[$name] = $family;
}


/**
 * Adds custom elements to the visual editor to be styled visually.
 * 
