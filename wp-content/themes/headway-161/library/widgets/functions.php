...
class SubscribeWidget extends WP_Widget {
    function SubscribeWidget() {
       	 $widget_ops = array('classname' => 'widget_headway_subscribe', 'description' => __( 'Displays subscribe via RSS and e-mail links.') );
        ...
        ?>
              <?php echo $before_widget ?>
				...
				<ul class="subscribe">
					...
					<?php if($email_url){ ?>
					<li class="email"><a href="<?php echo $email_url ?>" target="_blank">Subscribe via E-Mail</a></li>
					<?php } ?>
				</ul>
              ...
              <?php echo $before_widget; ?>
                  <?php if($instance['title']) echo $before_title
                      ...
                      . $after_title; ?>
						...
						<?php if($instance['feed']): ?><a href="<?php echo $instance['feed'] ?>" title="Subscribe via RSS!" target="_blank"><img src="<?php bloginfo('template_directory') ?>/media/images/social/feed.jpg" alt="Subscribe via RSS!" /></a><?php endif; ?>

						<?php if($instance['twitter']): ?><a href="<?php echo $instance['twitter'] ?>" title="Follow Me On Twitter!" target="_blank"><img src="<?php bloginfo('template_directory') ?>/media/images/social/twitter.jpg" alt="Follow Me On Twitter!" /></a><?php endif; ?>

						<?php if($instance['facebook']): ?><a href="<?php echo $instance['facebook'] ?>" title="Be my friend on Facebook!" target="_blank"><img src="<?php bloginfo('template_directory') ?>/media/images/social/facebook.jpg" alt="Follow Me On Twitter!" /></a><?php endif; ?>

						<?php if($instance['linkedin']): ?><a href="<?php echo $instance['linkedin'] ?>" title="Find me on LinkedIn!" target="_blank"><img src="<?php bloginfo('template_directory') ?>/media/images/social/linkedin.jpg" alt="Follow Me On Twitter!" /></a><?php endif; ?>

						<?php if($instance['youtube']): ?><a href="<?php echo $instance['youtube'] ?>" title="Subscribe to my channel on YouTube!" target="_blank"><img src="<?php bloginfo('template_directory') ?>/media/images/social/youtube.jpg" alt="Follow Me On Twitter!" /></a><?php endif; ?>

						<?php if($instance['vimeo']): ?><a href="<?php echo $instance['vimeo'] ?>" title="Subscribe to my channel on Vimeo!" target="_blank"><img src="<?php bloginfo('template_directory') ?>/media/images/social/vimeo.jpg" alt="Follow Me On Twitter!" /></a><?php endif; ?>

						<?php if($instance['stumbleupon']): ?><a href="<?php echo $instance['stumbleupon'] ?>" target="_blank"><img src="<?php bloginfo('template_directory') ?>/media/images/social/stumbleupon.jpg" alt="" /></a><?php endif; ?>

						<?php if($instance['friendfeed']): ?><a href="<?php echo $instance['friendfeed'] ?>" title="Subscribe to my FriendFeed!" target="_blank"><img src="<?php bloginfo('template_directory') ?>/media/images/social/friendfeed.jpg" alt="Follow Me On Twitter!" /></a><?php endif; ?>
		...
		if(count($leafs) > 0){												    	
			foreach($leafs as $leaf){ 													// Start foreach loop for every leaf/box.
				...
				if($leaf_config['type'] == 'sidebar' && !$leaf_options['duplicate-id']){
					...
					$widget_title[$leaf['id']] = ($leaf_options['sidebar-name']) ? 'ID: '.$leaf['id'].' &mdash; '.$leaf_options['sidebar-name'] : 'ID: '.$leaf['id'].' &mdash; '.base64_decode($leaf_config['title']);
