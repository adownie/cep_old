...
class HeadwayWrappers {
	...
	public static function get_all_wrappers() {
		...
		$wrappers_by_layout = HeadwaySkinOption::get_group('wrappers');
	...
	public static function get_layout_wrappers($layout_id, $include_design_editor_instances = false) {
		...
		$all_wrappers = HeadwaySkinOption::get_group('wrappers');
	...
	public static function get_wrapper($wrapper_id, $layout_id = null) {
		...
		$all_wrappers = self::get_all_wrappers();
		$wrapper = headway_get('wrapper-' . self::format_wrapper_id($wrapper_id), $all_wrappers);
		...
		/* Add in wrapper ID */
		$wrapper['id'] = 'wrapper-' . self::format_wrapper_id($wrapper_id);
		...
		public static function get_default_wrapper_settings($layout_id, $wrapper_id) {
			...
			return array_merge(self::$default_wrappers['wrapper-default'], array(
				'id' => 'wrapper-' . self::format_wrapper_id($wrapper_id),
				'layout' => $layout_id
			));
	...
	public static function add_wrapper($layout_id, $wrapper_settings, $skin_import = false) {
		...
		$last_wrapper_id = HeadwaySkinOption::get('last-id', 'wrappers');

		/* Prepare wrapper settings for new ID and clear out mirroring if it's not a skin import */
			if ( isset($wrapper_settings['layout']) )
				unset($wrapper_settings['layout']);

			if ( $skin_import ) {

				$new_wrapper_id = self::format_wrapper_id($wrapper_settings['id']);
		...
		/* Save wrappers array with new wrapper to layout */
			HeadwaySkinOption::set($layout_id, $existing_wrappers, 'wrappers');

		/* Save last wrapper ID back to DB */
			HeadwaySkinOption::set('last-id', $new_wrapper_id, 'wrappers');
	...
	public static function get_wrapper_mirror($wrapper_settings) {
		...
		$potential_wrapper_mirror = self::get_wrapper($wrapper_settings['mirror-wrapper']);

		if ( !empty($potential_wrapper_mirror['mirror-wrapper']) && self::get_wrapper($potential_wrapper_mirror['mirror-wrapper']) )
			return false;
	...
	public static function get_available_wrapper_id() {
		...
		return HeadwaySkinOption::get('last-id', 'wrappers', 1) + 1;
	...
	public static function register_wrapper_instances() {
		...
		foreach ( $all_wrappers as $layout_id => $layout_wrappers ) {
			...
			foreach ( $layout_wrappers as $layout_wrapper_id => $layout_wrapper_settings ) {
				...
				if ( self::get_wrapper_mirror($layout_wrapper_settings) )
					continue;

				$wrapper_name = headway_get('alias', $layout_wrapper_settings) ? 'Wrapper: ' . headway_get('alias', $layout_wrapper_settings) : 'Wrapper #' . HeadwayWrappers::format_wrapper_id($layout_wrapper_id);
				...
				HeadwayElementAPI::register_element_instance(array(
					...
					'name' => $wrapper_name,
					'selector' => '#' . $layout_wrapper_id . ', div.wrapper-mirroring-' . self::format_wrapper_id($layout_wrapper_id),
					'layout' => $layout_id
				));
	...
	public static function get_grid_width($wrapper) {
		...
		if ( $potential_wrapper_mirror = self::get_wrapper_mirror($wrapper) )
			$wrapper = $potential_wrapper_mirror;

		$column_width = headway_get('use-independent-grid', $wrapper) ? headway_get('column-width', $wrapper) : self::$global_grid_column_width;
		$gutter_width = headway_get('use-independent-grid', $wrapper) ? headway_get('gutter-width', $wrapper) : self::$global_grid_gutter_width;
