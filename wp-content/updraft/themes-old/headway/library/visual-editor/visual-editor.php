...
class HeadwayVisualEditor {
	...
	public static function save($options, $current_layout = false, $mode = false) {
		...
		if ( $mode == 'grid' )
			HeadwaySkinLayoutOption::set($current_layout, 'customized', true); 
					...
					foreach ( $methods as $method => $value ) {
						...
						switch ( $method ) {
							...
							case 'settings':
								...
								HeadwayBlocksData::update_block($block['layout'], $id, $block);
		...
		/* Wrappers */
			if ( $wrappers ) {
				...
				if ( headway_get('last-id', $wrappers) ) {
					...
					HeadwaySkinOption::set('last-id', $last_id, 'wrappers');

				}
		
				/* Save layout wrappers to dB */
				HeadwaySkinOption::set($current_layout, $wrappers, 'wrappers');
			...
			if ( $layout_options ) {
				...
				foreach ( $layout_options as $group => $options ) {
					...
					foreach ( $options as $option => $value ) {							
						HeadwaySkinLayoutOption::set($current_layout, $option, $value, $group);
					}
