...
"th", "tbody", "thead", "tfoot", "h1", "h2", "h3", "h4", "h5", "h6", "frame", "frameset", "script", "hgroup", "form", "label", "input", "textarea", "select", "fieldset", "legend"],
							iframe: true,
...
bindPanelInputs = function(context) {
	...
	/* Sliders */
		$('div.input-slider div.input-slider-bar', context).each(function() {
			...
			sliderInput.on('keyup', function(event) {
				...
				if ( this.value <= max && this.value >= min) {
					...
					$(this).parents('.input-slider').find('.input-slider-bar').slider( 'value', this.value );
					/* Handle hidden input */
					dataHandleInput($(this).parents('.input-slider').find('input.input-slider-bar-hidden'), this.value);
			...
			sliderInput.on('blur', function(event) {
				...
				if( this.value > max ) {
					alert('Exceeds Maximum: Enter a value between ' + min + ' and ' + max);
				} else if( this.value < min ) {
					alert('Exceeds Minimum: Enter a value between ' + min + ' and ' + max);
				} else {
					$(this).prev().text(this.value);
					$(this).parents('.input-slider').find('.input-slider-bar').slider( 'value', this.value );
					/* Handle hidden input */
		...
		if ( toggle.hasOwnProperty(val) ) {
			...
			/* Show */
				if ( typeof toggle[val].show == 'string' ) {
					...
					toShow.show();
					handleInputToggle(toShow.find('*[data-toggle]').not(input));
			...
			/* Hide */
				if ( typeof toggle[val].hide == 'string' ) {
					...
					handleInputToggleHideAll(toHide.find('*[data-toggle]').not(input));
					toHide.hide();
